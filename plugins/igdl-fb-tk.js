...
ruok({nomCom : "igdl" , categorie : "Téléchargement"},async (dest , zk , commandeOptions)=>{
  ...
  try {
    ...
    zk.sendMessage(dest,{video : {url : igvid.data.data.data[0].url},caption : "téléchargeur de video ig propulsé par *Ruok V1.0*",gifPlayback : false },{quoted : ms}) 
    }
    else {
        zk.sendMessage(dest,{image : {url : igvid.data.data.data[0].url},caption : "téléchargeur d'image ig propulsé par *Ruok V1.0*"})
    }
